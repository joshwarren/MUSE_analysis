...
from errors2_muse import apply_range, get_dataCubeDirectory,run_ppxf, \
	set_params
...
from compare_absorption3 import slitFoV
...
def compare_absortion(galaxy, O_sig=False, corr_lines='all'):
	...
	lines = ['H_beta', 'Fe5015', 'Mg_b', 'Fe5270', 'Fe5335', 'Fe5406', 
		'Fe5709', 'NaD']
	e_lines = ['e_H_beta', 'e_Fe5015', 'e_Mg_b', 'e_Fe5270', 'e_Fe5335', 
		'e_Fe5406', 'e_Fe5709', 'e_NaD']
	...
	mask = slitFoV(center, 4.1/abs(header['CD1_1'])*60**2, 
		2.5/header['CD2_2']*60**2, pa, instrument='muse')
...
if __name__=='__main__':
	...
	e_og = []
	line = []
	for gal in ['ic1459', 'ic4296', 'ngc1399']:
	...
	fig.savefig('%s/Data/lit_absorption/Ogando_comparison_muse.png' % (
		cc.base_dir))

	for l in np.unique(line):
		m = line == l

		print l, 'Offset:', np.nanmean(my[m] - og[m]), 'dispersion:', \
			np.nanstd(my[m] - og[m])
